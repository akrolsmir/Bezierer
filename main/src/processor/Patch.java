...
//	}
	 ...
	 */
	public List<Point[]> uniformTessellation(double step) {
		ArrayList<Point[]> quads = new ArrayList<Point[]>();

		for (double i = 0; i + step < 1 + 0.0001; i += step) {
			for (double j = 0; j + step < 1 + 0.0001; j += step) {
				Point[] quad = new Point[4];
				quad[0] = bezPatchInterp(i, j);
				quad[1] = bezPatchInterp(i + step, j);
				quad[2] = bezPatchInterp(i + step, j + step);
				quad[3] = bezPatchInterp(i, j + step);
				quads.add(quad);
			}
		}
